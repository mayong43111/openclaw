...
│                                       ▼                              │
│  ┌──────────────────────────────────────────────────────────┐        │
│  │  Ansible Worker（Function App + VNet 集成）              │        │
│  │  ┌─────────────┐                                        │        │
│  │  │ QueueTrigger│→ ansible-runner → deploy-vm.yml        │        │
│  │  └─────────────┘                                        │        │
...
│   │  └──────────┘  └──────────┘  └──────────┘                   │  │
│   │                                                              │  │
│   │  snet-worker (10.0.4.0/24) — Function App VNet 集成          │  │
│   └──────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────┘
...
- **Ansible Worker** = Function App + VNet 集成，可 SSH 到 VM 子网
...
| Ansible Worker  | Function App       | 监听队列，执行 Ansible playbook    |
| 状态存储        | Table Storage      | VM 记录（名称、状态、IP、token）   |
| Azure OpenAI    | Azure OpenAI Service| LLM 推理（gpt-5.4 模型部署）|
...
| 基础设施层 | VNet + NSG + AppGw + App Service + Function App | 一次性部署 (Terraform) | `make infra-apply` |
...
2. 任务入 Storage Queue
3. Function App（QueueTrigger）消费任务，执行 Ansible
...
```python
# Ansible Worker (Function App - QueueTrigger)
def handle_task(msg):
    task = json.loads(msg)
    if task['action'] == 'create':
        result = ansible_runner.run(
            ...
            extravars={
                ...
                'vm_admin_password': task['vm_admin_password'],
            }
        )
...
| 组件            | 选型                         | 理由                          |
|-----------------|------------------------------|-------------------------------|
| Console         | App Service (Python/Flask)   | PaaS，无需管理 VM，自带缩放  |
| 任务队列        | Azure Storage Queue          | 简单可靠，免费额度大          |
| Ansible Worker  | Function App (Python)        | QueueTrigger 自动消费，VNet 集成可 SSH |
| 状态存储        | Azure Table Storage          | 低成本，KV 查询足够           |

> Function App 需启用 **VNet 集成**（绑定 `snet-worker 10.0.4.0/24`），
> 使 Ansible 可通过内网 SSH 访问 `snet-vm` 中的 OpenClaw VM。
> Function App 执行 Ansible playbook 时间较长，需将 `functionTimeout`
> 设置为 `00:30:00`（Consumption Plan 最大 10 分钟，需使用 **Premium Plan**
> 或 **Dedicated Plan** 支持更长超时）。
...
Ansible Worker（Function App）调用此 playbook，传入 `vm_name` 和 `vm_admin_password`。
...
| 虚拟网络            | `10.0.0.0/16`                                         |
| VM 子网             | `snet-vm` — `10.0.1.0/24`（OpenClaw VM）              |
| Worker 子网         | `snet-worker` — `10.0.4.0/24`（Function App VNet 集成）|
| AppGw 子网          | `snet-appgw` — `10.0.2.0/24`                          |
| 网络安全组          | SSH: 仅 snet-worker + 跳板机；18789: 仅 AppGw 子网    |
| NAT Gateway         | `nat-{prefix}-openclaw`，绑定 snet-vm（VM 出站访问 AOAI 需要） |
| 公共 IP             | 静态，Standard SKU（App Gateway + NAT Gateway 各一个） |
| Storage Account     | Queue（vm-tasks）+ Table（vm-records）                 |
| App Service         | Console（Flask/FastAPI），Web UI + REST API            |
| Function App        | Ansible Worker，Premium Plan，VNet 集成到 snet-worker  |
| Azure OpenAI        | `auto-gen-chat`（已有资源），部署 gpt-5.4   |
...
> Ansible Worker（Function App）通过 VNet 集成的 `snet-worker` 子网 SSH 到 `snet-vm` 子网。
...
使用已有的 `auto-gen-chat` AOAI 资源（`aif-auto-gen-chat`，非 Terraform 部署）：

| 项目                   | 配置                                                    |
|------------------------|--------------------------------------------------------|
| Azure OpenAI Account   | `aif-auto-gen-chat`（`rg-auto-gen-chat`）               |
...
使用 **Custom Provider** + 手动 JSON 配置接入 Azure OpenAI。
...
OpenClaw VM（NSSM 服务以 SYSTEM 运行）
       ...
       ▼
  Azure OpenAI (aif-auto-gen-chat)
...
AOAI 资源强制 `disableLocalAuth: true`，API Key 不可用。
...
当前使用已有 `auto-gen-chat` 资源，无需 Terraform 部署 AOAI。
如未来需独立部署，参考如下配置：
...
| Azure OpenAI         | `auto-gen-chat`（已有资源）    |
...
| 层                   | 控制措施                                              |
|----------------------|-------------------------------------------------------|
| 网络（NSG）          | SSH: 仅 Worker 子网（`10.0.4.0/24`）+ 跳板机 VNet     |
...
| Worker 网络          | Function App VNet 集成（`snet-worker`），内网 SSH 到 VM |
...
```
  ...
  console-deploy  部署 Console App Service 代码
  worker-deploy   部署 Ansible Worker Function App 代码
  show-ca         显示 CA 证书详情
...
- **MCAPSGov 策略**：管理组级别策略对 Terraform 部署的 AOAI 资源强制
  `disableLocalAuth=true`。当前使用已有 `auto-gen-chat` 资源（`localAuth` 已启用）规避。
- **AOAI Endpoint**：`https://aif-auto-gen-chat.openai.azure.com/`
...
- [ ] remove-vm.yml（退役 VM + 清理路由 + 记录）
- [ ] Console App Service（Web UI + REST API）
- [ ] Worker Function App（QueueTrigger + Ansible 异步执行）
...
| Console 创建 VM 超时             | Function App 超时。确认使用 Premium Plan（支持 30 min）|
| 队列消息未被消费                 | Function App 未启动或 VNet 集成未生效。检查 Function 日志 |
| Worker 无法 SSH 到 VM            | snet-worker 子网未委派给 Function App，或 NSG 规则缺失 |
| AOAI 返回 401 Unauthorized       | API Key 错误或未配置。检查 `openclaw.json` 中 `models.providers.auto-gen-chat.apiKey` 字段 |
| AOAI 返回 403 Forbidden          | API Key 无权访问该模型部署，检查 AOAI 资源访问控制   |
...
| AOAI `AuthenticationTypeDisabled` | MCAPSGov 策略禁止 API Key。更换到 `localAuth` 已启用的 AOAI 资源 |
